...
export default function TerminalContent() {
  ...
  const renderFileTree = (items: FileItem[], level = 0) => (
    items.map(item => (
      <div key={item.id}>
        <div
          className={`flex items-center px-2 py-1 cursor-pointer hover:bg-gray-700 ${selectedItem?.id === item.id ? 'bg-gray-700' : ''}`}
    ...
    <div className="h-full bg-gray-900 text-gray-100 font-mono text-sm flex">
      <div className="w-1/2 flex flex-col border-r border-gray-700">
        <div className="flex-1 overflow-y-auto border-b border-gray-700">
          <div className="bg-gray-800 px-3 py-2 border-b border-gray-700 text-xs text-gray-400 uppercase">Files</div>
          <div className="p-2">{renderFileTree(mockFiles)}</div>
        </div>
        <div className="h-1/3 overflow-y-auto">
          <div className="bg-gray-800 px-3 py-2 border-b border-gray-700 text-xs text-gray-400 uppercase">Recent</div>
          <div className="p-2">
            {recentFiles.map(file => (
              <div
                key={file.id}
                className={`px-2 py-1 cursor-pointer hover:bg-gray-700 ${selectedItem?.id === file.id ? 'bg-gray-700' : ''}`}
                onClick={() => {
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          {selectedItem ? (
            ...
            <div className="text-gray-500 text-center mt-10">Select a file to view its contents</div>
          )}
        </div>
      </div>
    </div>
